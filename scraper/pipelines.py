...
class GamesReviewsPipelineJSON:
    ...
    def from_crawler(cls, crawler):
        ...
        dst_dir = os.path.join(settings.get("DST_DIR", '.'), crawler.spider.name)
    ...
    def process_item(self, item, spider):
        ...
        if len(self.items) >= self.max_num_items:
            self.save_json()
            spider.crawler.engine.close_spider(self, reason='Scrapping finished.')
...
class GamesReviewsPipelineMongoDB:
    ...
    def from_crawler(cls, crawler):
        settings = crawler.settings
        max_num_items = settings.get("MAX_NUM_ITEMS", 15)
        coll = get_collection(get_db(settings, drop=False), settings.get('MONGODB_COLLECTION'))
